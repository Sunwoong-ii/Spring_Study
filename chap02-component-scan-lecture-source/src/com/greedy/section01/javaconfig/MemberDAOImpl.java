 ...
 * 이름(id)를 설정하지 않으면 class의 앞글자를 소문자로 하여 bean으로 생성한다.
 * 
 ...
 * @Controller : Web MVC 코드에서 사용되는 어노테이션으로 @RequestMapping 어노테이션은 해당 어노테이션이 작성된 클래스
 * 				 내에서만 사용가능하다.
 * @Service : 비지니스 로직이 작성된 클래스에 사용한다. 추가적인 기능은 없지만 추가적인 기능을 제공할 가능성이 있으니 계층을 명시할 때 사용한다.
 * @Repository : 플랫폼별 예외를 포착하여 PersistenceExceptionTransacttionPostProcesser가 DataAccessException 으로
 * 				 변환하여 다시 발생한다.
...
public class MemberDAOImpl implements MemberDAO{
	...
	public MemberDAOImpl() {
		...
		memberMap.put(2, new MemberDTO(2, "user02", "pass02", "김선웅"));
	}
	
//	매개변수로 전달받은 회원 번호를 map에서 조회 후 리턴해주는 메소드
	@Override
